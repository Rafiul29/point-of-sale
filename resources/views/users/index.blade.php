...
<div class="flex flex-col gap-10" x-data="userManager()">
    ...
    <div class="flex items-center justify-between">
        ...
        <button @click="openCreateModal()" class="flex items-center gap-2 rounded-2xl bg-indigo-600 px-6 py-3 text-sm font-black text-white shadow-lg shadow-indigo-500/30 transition-all hover:bg-indigo-700 hover:scale-[1.02]">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-slate-50 text-left">
                            <th class="px-6 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">User</th>
                            <th class="px-6 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Email</th>
                            <th class="px-6 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Role</th>
                            <th class="px-6 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Joined</th>
                            <th class="px-6 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="transition-colors hover:bg-slate-50/50">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-widest
                                    {{ $user->role === 'Admin' ? 'bg-indigo-50 text-indigo-700' : 'bg-purple-50 text-purple-700' }}">
                                    <i class="fas {{ $user->role === 'Admin' ? 'fa-user-shield' : 'fa-cash-register' }} text-[8px]"></i>
                                    {{ $user->role }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-xs text-slate-400 font-semibold">{{ $user->created_at->format('M d, Y') }}</td>
                            <td class="px-6 py-4">
        ...
        <div class="relative w-full max-w-lg rounded-[2.5rem] bg-white shadow-2xl" x-transition>
            <div class="p-10">
                <div class="mb-8 flex items-center justify-between">
                    ...
                    <button @click="showModal = false" class="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors">
                        <i class="fas fa-times"></i>
                    </button>
                </div>

                <form :action="editing ? '/users/' + form.id + '?_method=PUT' : '/users'" method="POST" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">
                            Password <span x-show="editing" class="normal-case font-medium text-slate-300">(leave blank to keep current)</span>
                        </label>
                    ...
                    <div class="rounded-2xl p-4 transition-colors"
                        ...
                        <p class="text-[10px] font-black uppercase tracking-widest mb-2" :class="form.role === 'Admin' ? 'text-indigo-500' : 'text-purple-500'">
                            Permissions
                        </p>
                        <ul class="space-y-1 text-xs font-semibold" :class="form.role === 'Admin' ? 'text-indigo-700' : 'text-purple-700'">
                            <template x-if="form.role === 'Admin'">
                                <div class="space-y-1">
                                    <li class="flex items-center gap-2"><i class="fas fa-check text-emerald-500"></i> Full dashboard & reports</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check text-emerald-500"></i> Inventory & product management</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check text-emerald-500"></i> User & settings management</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check text-emerald-500"></i> POS terminal access</li>
                                </div>
                            </template>
                            <template x-if="form.role === 'Cashier'">
                                <div class="space-y-1">
                                    <li class="flex items-center gap-2"><i class="fas fa-check text-emerald-500"></i> Personal sales dashboard</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check text-emerald-500"></i> POS terminal access</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-check text-emerald-500"></i> View customers</li>
                                    <li class="flex items-center gap-2"><i class="fas fa-times text-rose-400"></i> No inventory/admin access</li>
                                </div>
                            </template>
                        </ul>
                    </div>
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="showDeleteModal = false">
        ...
        <div class="relative w-full max-w-sm rounded-[2.5rem] bg-white p-10 shadow-2xl text-center" x-transition>
            <div class="flex h-20 w-20 items-center justify-center rounded-full bg-rose-50 text-4xl text-rose-500 mx-auto mb-6">
            ...
            <form :action="'/users/' + deleteTarget.id" method="POST">
                ...
                @method('DELETE')
                <div class="flex items-center gap-3">
                    <button type="submit" class="flex-1 rounded-2xl bg-rose-600 py-4 text-sm font-black text-white shadow-lg shadow-rose-500/30 transition-all hover:bg-rose-700">
                        Yes, Delete
                    </button>
