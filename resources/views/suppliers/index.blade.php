...
<div class="flex flex-col gap-8 p-4 sm:p-6 lg:p-8">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="rounded-4xl bg-linear-to-br from-indigo-900 via-indigo-800 to-blue-900 border border-indigo-700 p-5 sm:p-7 lg:p-8 shadow-2xl text-white sticky top-20 lg:top-24">
                <h3 class="text-xl sm:text-2xl font-extrabold mb-5 text-white tracking-tight" id="supplier-form-title">Onboard Supplier</h3>
                <form id="supplier-form" action="{{ route('suppliers.store') }}" method="POST" class="space-y-5">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-1">
                            ...
                            <input id="supplier-name-input" type="text" name="name" class="w-full border border-white/30 bg-white/10 py-3.5 px-4 text-sm font-semibold rounded-2xl text-white placeholder:text-indigo-200 transition-all focus:border-blue-300 focus:bg-white/20 focus:outline-none focus:ring-4 focus:ring-blue-300/40" required placeholder="e.g. Acme Corp">
                        </div>
                        <div class="space-y-1">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-indigo-400 ms-1">Email Connection</label>
                            <input id="supplier-email-input" type="email" name="email" class="w-full border border-white/30 bg-white/10 py-3.5 px-4 text-sm font-semibold rounded-2xl text-white placeholder:text-indigo-200 transition-all focus:border-blue-300 focus:bg-white/20 focus:outline-none focus:ring-4 focus:ring-blue-300/40" placeholder="contact@supplier.com">
                        </div>
                        <div class="space-y-1">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-indigo-400 ms-1">Contact Phone</label>
                            <input id="supplier-phone-input" type="text" name="phone" class="w-full border border-white/30 bg-white/10 py-3.5 px-4 text-sm font-semibold rounded-2xl text-white placeholder:text-indigo-200 transition-all focus:border-blue-300 focus:bg-white/20 focus:outline-none focus:ring-4 focus:ring-blue-300/40" placeholder="+1 (555) 000-0000">
                        </div>
                        <div class="space-y-1">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-indigo-400 ms-1">Physical HQ</label>
                            <textarea id="supplier-address-input" name="address" rows="3" class="w-full border border-white/30 bg-white/10 py-3.5 px-4 text-sm font-semibold rounded-2xl text-white placeholder:text-indigo-200 transition-all focus:border-blue-300 focus:bg-white/20 focus:outline-none focus:ring-4 focus:ring-blue-300/40" placeholder="Street, City, Country..."></textarea>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <button id="supplier-form-submit" class="w-full py-4 rounded-2xl bg-indigo-500 font-extrabold text-white transition-all hover:bg-indigo-400 hover:shadow-xl hover:shadow-indigo-500/20 active:scale-95" type="submit">
                            <i class="fas fa-handshake mr-2 opacity-70"></i> Register Supplier
                        </button>
                        <button id="supplier-form-cancel" type="button" class="hidden w-full py-4 rounded-2xl border border-white text-white hover:bg-white/10" aria-label="Cancel edit">
                            Cancel
                        </button>
                    </div>
                </form>
                            ...
                            <tr class="group hover:bg-slate-50/50 transition-all duration-300">
                                ...
                                <td class="px-10 py-6 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-100">
                                        <button type="button" class="edit-supplier flex h-10 w-10 items-center justify-center rounded-xl bg-slate-900 text-slate-100 hover:bg-indigo-600 hover:text-white transition-all shadow-lg hover:shadow-slate-900/20" data-id="{{ $supplier->id }}" data-name="{{ $supplier->name }}" data-email="{{ $supplier->email }}" data-phone="{{ $supplier->phone }}" data-address="{{ $supplier->address }}" aria-label="Edit supplier">
